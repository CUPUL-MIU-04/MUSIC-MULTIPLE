### CONDITIONING.md
```markdown
# Music Multiple conditioning modules

Music Multiple provides a
...
For now, we support 3 main types of conditioning within Music Multiple:
...
modules in Music Multiple and are used internally in the language model to process the
...
The `BaseConditioner` torch module is the base implementation for all conditioners in Music Multiple.
...
All metadata-enabled datasets to use for conditioning in Music Multiple inherits
the [`audiocraft/data/info_audio_dataset.InfoAudioDataset`](../audiocraft/data/info_audio_dataset.py) class
...
Music Multiple currently provides two text conditioners:
...
We provide a Classifier Free Guidance implementation in Music Multiple. With the classifier free
