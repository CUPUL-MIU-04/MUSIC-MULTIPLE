...
"""Music Multiple - Dataset Resampling Script
"""
import argparse
...
from music_multiple.data.audio import audio_read, audio_write  # ✅ Cambiado
from music_multiple.data.audio_dataset import load_audio_meta, find_audio_files  # ✅ Cambiado
from music_multiple.data.audio_utils import convert_audio  # ✅ Cambiado
from music_multiple.environment import AudioCraftEnvironment  # ✅ Cambiado


def read_txt_files(path: tp.Union[str, Path]):
    with open(args.files_path) as f:
        lines = [line.rstrip() for line in f]
        print(f"🎵 Music Multiple - Read {len(lines)} lines in .txt")
        lines = [line for line in lines if Path(line).suffix not in ['.json', '.txt', '.csv']]
        print(f"🎵 Music Multiple - Filtered and keep {len(lines)} audio files from .txt")
        return lines
...
def read_egs_files(path: tp.Union[str, Path]):
    ...
    if path.is_dir():
        if (path / 'data.jsonl').exists():
            ...
            raise ValueError("🎵 Music Multiple - Don't know where to read metadata from in the dir. "
...
def process_dataset(args, n_shards: int, node_index: int, task_index: tp.Optional[int] = None):
    ...
    if args.files_path is None:
        ...
        if files_path.suffix == '.txt':
            print(f"🎵 Music Multiple - Reading file list from .txt file: {args.files_path}")
            lines = read_txt_files(args.files_path)
        else:
            print(f"🎵 Music Multiple - Reading file list from egs: {args.files_path}")
            lines = read_egs_files(args.files_path)

    total_files = len(lines)
    print(
        f"🎵 Music Multiple - Total of {total_files} files processed with {n_shards} shards. " +
    ...
    for idx, line in tqdm.tqdm(enumerate(lines)):
        ...
        assert path.startswith(str(root_path)), \
            f"🎵 Music Multiple - Mismatch between path and provided root: {path} VS {root_path}"
        ...
        try:
            ...
            print(f"🎵 [{idx}] Processing: {out_path} from {path}")
            ...
            if out_channels > 2:
                print(f"🎵 Music Multiple - Mix has more than two channels: {out_channels}, enforcing 2 channels")
                out_channels = 2
            ...
            if metadata_path.exists():
                ...
                print(f"🎵 Music Multiple - No metadata found at {str(metadata_path)}")
            out_done_token.touch()
        except Exception as e:
            print(f"🎵 Music Multiple - Error processing file line: {line}, {e}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Music Multiple - Resample dataset with SLURM.")
    parser.add_argument(
        "--log_root",
        type=Path,
    ...
    parser.add_argument("--xp_name", type=str, default="music_multiple_dataset")
    parser.add_argument(
        "--nodes",
        type=int,
        default=4,
    ...
    if args.files_path is None:
        print("🎵 Music Multiple - Warning: --files_path not provided, not recommended when processing more than 10k files.")
    if args.debug:
        print("🎵 Music Multiple - Debugging mode")
        process_dataset(args, n_shards=n_shards, node_index=0, task_index=0)
    else:
        log_folder = Path(args.log_root) / args.xp_name / '%j'
        print(f"🎵 Music Multiple - Logging to: {log_folder}")
        ...
        with executor.batch():
            for node_index in range(args.nodes):
                ...
                jobs.append(job)
        for job in jobs:
            print(f"🎵 Music Multiple - Waiting on job {job.job_id}")
            job.results()