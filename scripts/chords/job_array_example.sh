#!/bin/zsh
# Music Multiple - Chord Extraction Job Array
# SBATCH example script for parallel chord processing

#SBATCH --job-name=music_multiple_chords
...
conda activate music_multiple_env

echo "🎵 Music Multiple - Starting chord extraction job ${SLURM_ARRAY_TASK_ID}"

# run extraction of chords in job array
python scripts/chords/extract_chords.py \
    --src_jsonl_file /path/to/parsed/filepaths_${SLURM_ARRAY_TASK_ID}.jsonl \
    --target_output_dir /target/directory/to/save/chords/to \
    --path_to_pre_defined_map /path/to/predefined/chord_to_index_mapping.pkl

echo "🎵 Music Multiple - Chord extraction job ${SLURM_ARRAY_TASK_ID} completed"