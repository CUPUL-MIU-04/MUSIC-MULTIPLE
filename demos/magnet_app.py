...
def ui_full(launch_kwargs):
    with gr.Blocks() as interface:
        gr.Markdown(
            """
            # Music Multiple - MAGNeT Fast Music Generation
            **Music Multiple** presents MAGNeT - Ultra-fast text-to-music generation using a single non-autoregressive transformer.
            
            Based on the research: ["Masked Audio Generation using a Single Non-Autoregressive Transformer"] (https://huggingface.co/papers/2401.04577)

            *Part of the Music Multiple ecosystem - High-speed music generation platform*
            """
        )
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    model = gr.Radio(['facebook/magnet-small-10secs', 'facebook/magnet-medium-10secs',
                                     ...
                                     label="Model Version", value='facebook/magnet-small-10secs', interactive=True)
                    model_path = gr.Text(label="Custom Model Path (advanced)") 
                with gr.Row():
                    span_score = gr.Radio(["max-nonoverlap", PROD_STRIDE_1],
                                       label="Span Scoring Method", value=PROD_STRIDE_1, interactive=True)       
                with gr.Row():
                    decoding_steps1 = gr.Number(label="Stage 1 Steps", value=20, interactive=True)
                    decoding_steps2 = gr.Number(label="Stage 2 Steps", value=10, interactive=True)
                    decoding_steps3 = gr.Number(label="Stage 3 Steps", value=10, interactive=True)
                    decoding_steps4 = gr.Number(label="Stage 4 Steps", value=10, interactive=True)
                with gr.Row():
                    ...
                    max_cfg_coef = gr.Number(label="Max CFG", value=10.0, minimum=0, interactive=True)
                    min_cfg_coef = gr.Number(label="Min CFG", value=1.0, minimum=0, interactive=True)                
            with gr.Column():
                output = gr.Video(label="Generated Music Video - Variation 1")
        ...
        gr.Markdown(
            """
            ### 🎵 About Music Multiple - MAGNeT

            **Music Multiple** brings you MAGNeT - Ultra-fast music generation technology:

            #### 🎼 Music Generation
            MAGNeT models generate short music extracts based on textual descriptions with incredible speed.
            Available durations: **10 seconds** or **30 seconds** of music.

            **Model Variants:**
            1. **magnet-small-10secs** - 300M parameters, 10-second music
            2. **magnet-medium-10secs** - 1.5B parameters, 10-second music  
            3. **magnet-small-30secs** - 300M parameters, 30-second music
            4. **magnet-medium-30secs** - 1.5B parameters, 30-second music

            #### 🔊 Sound Effect Generation  
            Audio-MAGNeT models generate 10-second sound effects based on descriptions.

            **Sound Effect Models:**
            1. **audio-magnet-small** - 300M parameters, 10-second effects
            2. **audio-magnet-medium** - 1.5B parameters, 10-second effects

            #### ⚡ Speed Advantage
            MAGNeT uses non-autoregressive generation, making it significantly faster than traditional 
            autoregressive models while maintaining high quality.
